...
pub fn signed_transaction_to_hex<'a>(
    ...
    let tx_aux = signed_transaction_to_tx_aux(ctx, signed_transaction, tendermint_address, features)?;
...
pub fn signed_transaction_to_tx_aux<'a>(
    ...
    match features {
        ...
        Features::MockAbci => {
            to_mock_abci_tx_aux(ctx, signed_transaction, &tendermint_address)
        }
        Features::MockObfuscation => {
            to_mock_tx_aux(ctx, signed_transaction, &tendermint_address)
        }
    }
}

pub fn tx_aux_to_hex<'a>(
    ctx: &mut FunctionContext<'a>,
    tx_aux: TxAux
) -> JsResult<'a, JsBuffer> {
...
fn to_tx_aux_websocket(
    ...
    tx_obfuscation.encrypt(signed_transaction)
        .chain_neon(
            ctx,
            "Unable to encrypt transaction"
        )
}
