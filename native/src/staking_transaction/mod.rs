...
pub fn register_staking_transaction_module(ctx: &mut ModuleContext) -> NeonResult<()> {
    ...
    let withdraw_unbonded_transaction_to_witness_fn =
        JsFunction::new(ctx, withdraw_unbonded_transaction_to_witness)?;
    js_object.set(
        ctx,
        "withdrawUnbondedTransactionToWitness",
        withdraw_unbonded_transaction_to_witness_fn,
    )?;
